...
SELECT
   ...
   FLOOR(SQRT(SALARY)) 	  FUNC_FLOOR2,
   CEIL(SQRT(SALARY)) 	  FUNC_CEIL2
FROM EMPLOYEES;
...
desc db_funcionario;t

--ex1
select INITCAP(NM_FUNCIONARIO) from db_funcionario;

--ex2

select NM_FUNCIONARIO, DT_NASCIMENTO from db_funcionario;

select NM_FUNCIONARIO,round((months_between(sysdate,to_date(DT_NASCIMENTO,'dd/mm/rrrr')))/12)as idade 
from db_funcionario; 

--ex 3 

select NM_FUNCIONARIO,replace(replace(mod(CEIL((months_between(sysdate,to_date(DT_NASCIMENTO,'dd/mm/rrrr')))/12),2),'1','Impar'),'0','par') as idade
from db_funcionario ; 

--ex4 

select NM_FUNCIONARIO,ceil(VL_SALARIO_BRUTO) from db_funcionario ; 

--------desafio
--1
select NM_FUNCIONARIO
from db_funcionario where round((months_between(sysdate,to_date(DT_NASCIMENTO,'dd/mm/rrrr')))/12)>=52;

--2

select NM_FUNCIONARIO,(55- (round((months_between(sysdate,to_date(DT_NASCIMENTO,'dd/mm/rrrr')))/12)))as faltamAnos
from db_funcionario where round((months_between(sysdate,to_date(DT_NASCIMENTO,'dd/mm/rrrr')))/12)<52; 

--3

select NM_FUNCIONARIO,trunc((VL_SALARIO_BRUTO*0.70),2) as SalarioAposentado from db_funcionario ; 
